...
import { getDesignByCode, createPromotion } from '../../../actions/promotionActions';
...
class addPromotionD extends React.Component {
	...
	submitHandler = () => {
		const validate = (promotion, item, file) => {
			...
			if (validateFile(file) !== 'Something Wrong' && validateItem.length === 0 && promotion.title !== '') {
				return true;
			} else return false;
		};

		if (!validate(this.state.marketHeader, this.state.designItem, this.state.shareholders)) {
								...
								<div class="form-group col-md-6 row">
									...
									<div class="col-8">
										<input
											...
											class="form-control"
											id="requestdate"
											placeholder={moment(new Date().toDateString()).format('DD/MM/YYYY')}
											disabled
										/>
									</div>
								</div>
										...
										<tr>
											<td>
												<input
													...
													placeholder={this.state.designItem[index].product_name}
													disabled
												/>
											</td>
											<td>
...
export default connect(mapStateToProps, { getDesignByCode, getDesignItem, createPromotion })(addPromotionD);
