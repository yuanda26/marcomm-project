...
export const deleteRole = (param, modalStatus) => (dispatch) => {
	...
	axios(options)
		.then((res) => {
			...
			modalStatus(1, `Data deleted!, Data role with code ${param} has been deleted!`);
		})
		.catch((error) => {
			dispatch({
				type: DEL_ROLE,
			...
			modalStatus(2, `Invalid!, Data role with code  ${param} is used in other account!`);
		});
};
...
export const createRole = (body, modalCallback) => (dispatch) => {
	...
	axios(option)
		.then((response) => {
			...
			modalCallback(1, `Data Saved! New role with code RO-${response.data.message + 1} has been added!`);
		})
		.catch((error) => {
			dispatch({
				type: ADD_ROLE,
...
export const putRole = (body, modalStatus) => (dispatch) => {
	...
	axios(option)
		.then((res) => {
			...
			modalStatus(1, `Data Updated!, data with code ${body.code} has been updated!`);
		})
		.catch((error) => {
			dispatch({
				type: PUT_ROLE,
