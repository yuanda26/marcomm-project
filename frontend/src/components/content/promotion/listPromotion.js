...
class ListPromotion extends React.Component {
  ...
  search = () => {
    let patt = {
      ...
      request_date: new RegExp(
        moment(this.state.search.request_date).format("DD/MM/YYYY")
      ),
      request_by: new RegExp(this.state.search.request_by, "i"),
      assign_to: new RegExp(this.state.search.assign_to, "i"),
      status: new RegExp(this.showAssign(this.state.search.status), "i"),
      created_date: new RegExp(
        moment(this.state.search.created_date).format("DD/MM/YYYY")
      ),
      created_by: new RegExp(this.state.search.created_by)
    };
    let newResult = this.state.hasil

      // patt.created_date.test(
      //   moment(content.created_date).format("DD/MM/YYYY")
      // ) ||
      // patt.assign_to.test(content.assign_to) ||
      .map(content => {
        if (
  ...
  showstatus(code) {
    if (code == 1) return "Submitted";
    else if (code == 2) return "In Progress";
    else if (code == 3) return "Done";
    else if (code == 0) return "Rejected";
  }
                      ...
                      return (
                        <TableRow key={row._id}>
                          ...
                          <TableCell>
                            ...
                            {/* <Link to="#">
                              <DeleteOutlinedIcon onClick={() => {this.deleteModalHandler(row._id)}}/>
                            </Link> */}
                          </TableCell>
                        </TableRow>
                      );
